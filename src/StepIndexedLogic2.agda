...
{-
  ◇→ : ∀{k} → 𝒫 ⊢ᵒ (◇ᵒ k (ϕ →ᵒ ψ))  →  𝒫 ⊢ᵒ (◇ᵒ k ϕ) →ᵒ (◇ᵒ k ψ)
  ◇→ {𝒫}{ϕ}{ψ}{zero} ◇ϕ→ψ = ▷→ {𝒫}{ϕ}{ψ} ◇ϕ→ψ
  ◇→ {𝒫}{ϕ}{ψ}{suc k} ◇▷ϕ→ψ =
    --let ▷◇ϕ→ψ = substᵒ{𝒫 = 𝒫} (≡ᵒ-sym (▷◇≡◇▷{ϕ = ϕ →ᵒ ψ} k)) ◇▷ϕ→ψ in
    ◇→ {𝒫}{▷ᵒ ϕ}{▷ᵒ ψ}{k} {!!} 
  ...
  ◇→◇ : ∀{𝒫}{P Q : Setᵏ}{k}
     ...
     → P ∷ 𝒫 ⊢ᵒ Q
       ------------
     → 𝒫 ⊢ᵒ ◇ᵒ k Q
  ◇→◇ {𝒫} {P} {Q} {zero} ▷P P⊢Q = ▷→▷{𝒫} {P} {Q} ▷P P⊢Q 
  ◇→◇ {𝒫} {P} {Q} {suc k} ◇kP P⊢Q =
    let IH = ◇→◇ {𝒫} {{!!}}{{!!}} {k} {!!} P⊢Q in
    {!!}

  #◇k≡ : ∀ {ϕ}{δ}{k}{i} → # (◇ᵒ{[]}{[]} k ϕ) δ i ≡ ∀ (j : ℕ) → ((k + j) < i) → # ϕ δ j
  #◇k≡ {ϕ} {δ} {zero} {i} = refl
  #◇k≡ {ϕ} {δ} {suc k} {i} =
    let IH = #◇k≡ {▷ᵒ ϕ} {δ} {k} {i} in
    {!!}
-}
