...
\frame{
  \frametitle{Recipe: extend $𝒱$ and $ℰ$ to open terms}
    ...
    \begin{align*}
      ...
      \Gamma \models^{𝒱} V : A &=
        \forall \sigma,
        \mathcal{G}(\Gamma)(\sigma) \mathop{⊢ᵒ} 𝒱(A)(\sigma(V))\\
      \Gamma \models M : A &=
        \forall \sigma,
...
\frame{
  ...
  \begin{proof}
    ...
    We need to show that
    \[
    \mathcal{G}(Γ)(σ) ⊢ᵒ 𝒱(ℕ)(σ(\suc{V}))
    \]
    which is equivalent to 
    \[
    \mathcal{G}(Γ)(σ) ⊢ᵒ 𝒱(ℕ)(σ(V))
    \]
...
\frame{
  ...
  \begin{proof}
    ...
    $\mathcal{G}(A→B, Γ)(σ') ⊢ᵒ 𝒱(A → B, σ'(V))$ 
    where $σ' = (μ\,V',σ)$, \\
    and therefore \\
    $\mathcal{G}(Γ)(σ) ⊢ᵒ 𝒱(A→B)(μ\,V') ⇒ 𝒱(A → B)(σ'(V))$. \\
    Together with (IH), we have \\
    $\mathcal{G}(Γ)(σ) ⊢ᵒ \mathop{▷ᵒ} 𝒱(A → B)(σ'(V))$\\
