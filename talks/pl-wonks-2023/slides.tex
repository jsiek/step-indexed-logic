...
\frame{
  ...
  \item Define two relations that generalize the theorem you'd like to
    prove: one on types and closed values $\mathcal{V}(A,V)$
    and the other on types and closed terms $\mathcal{E}(A,M)$.
  \item Extend the $\mathcal{E}$ relation to open terms.\\
    (Let $\sigma$ be a substitution, i.e. a mapping of variables to terms,
