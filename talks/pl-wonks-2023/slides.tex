...
\fbox{$\Gamma \vdash^𝒱 V : A$}
\begin{gather*}
  \inference{}
            {\Gamma \vdash^𝒱 \zero : \Nat}
            \quad
  \inference{\Gamma \vdash^𝒱 V : \Nat}
            {\Gamma \vdash^𝒱 \suc{V} : \Nat}
            \\[2ex]
  \inference{\Gamma,A \vdash^𝒱 N : B}{\Gamma \vdash^𝒱 \lam{N} : A \to B}
  \inference{\Gamma,A \to B \vdash^𝒱 V : A \to B}{\Gamma \vdash^𝒱 \mu{V} : A \to B}
\end{gather*}

\fbox{$\Gamma \vdash M : A$}
\begin{gather*}
  \inference{\Gamma \vdash^{𝒱} V : A}{\Gamma \vdash V : A}
  \quad
  \inference{}
            {Γ \vdash i : Γ_i}
            \\[2ex]
...
\frame{
  ...
  \item Define two functions that generalize the theorem you'd like to
    prove: one maps types to a predicate on closed values $𝒱(A)(V)$
    and the other maps types to a predicate on closed terms $ℰ(A)(M)$.
  \item Extend the $𝒱$ and $ℰ$ functions to open terms:
    \begin{align*}
      𝒢(\Gamma)(\sigma) &=
        \forall A_i \in \Gamma, 𝒱(A_i)(\sigma_i)  \\
        \Gamma \models^{𝒱} V : A &=
           \forall \sigma,
           𝒢(\Gamma)(\sigma) \text{ implies }
           𝒱(A)(\sigma(V)) \\
        \Gamma \models M : A &=
           \forall \sigma,
           𝒢(\Gamma)(\sigma) \text{ implies }
  ...
  \item Prove the Fundamental Lemma, that\\
    (1) $\Gamma \vdash^{𝒱} V : A$ implies $\Gamma \models^{𝒱} V : A$ and\\
...
\frame{
  ...
  ℰ(A)(M) = 
  \begin{array}{l}
    𝒱(A)(M) \mathop{\mathsf{or}} ∃ M', M \longrightarrow M' \\
    \mathop{\mathsf{and}} ∀ M', M \longrightarrow M' \Rightarrow ℰ(A)(M')
  \end{array}
  \]
  \fbox{$𝒱(A)(V) : 𝔹$}
  \begin{align*}
    𝒱(\Nat)(\zero) &= \mathsf{true} \\
    𝒱(\Nat)(\suc{V}) &= 𝒱(\Nat)(V) \\
    𝒱(A → B)(\lam{N}) &= 
      \forall W, 𝒱(A)(W) \Rightarrow ℰ(B)(N[W]) \\
    𝒱(A → B)(μ V) &= 𝒱(\rd{A → B})(V[μ V]) \\
    𝒱(A)(V) &= \mathsf{false} \qquad \text{otherwise} 
  \end{align*}
...
\frame{
  ...
  ℰ(A)(M) = 
  \begin{array}{l}
    𝒱(A)(M) \OR \EX M', M \longrightarrow M' \\
    \AND \ALL M', M \longrightarrow M' \Rightarrowᵒ \mathop{▷ᵒ} ℰ(A)(M')
  \end{array}
  \]
  \fbox{$𝒱(A)(V) : ℕ → 𝔹$}
  \begin{align*}
    𝒱(\Nat)(\zero) &= \mathsf{true}ᵒ \\
    𝒱(\Nat)(\suc{V}) &= 𝒱(\Nat)(V) \\
    𝒱(A → B)(\lam{N}) &= 
      \forallᵒ W, \mathop{▷ᵒ} 𝒱(A)(W) \Rightarrowᵒ \mathop{▷ᵒ} ℰ(B)(N[W]) \\
    𝒱(A → B)(μ V) &= \mathop{▷ᵒ} 𝒱(A → B)(V[μ V]) \\
    𝒱(A)(V) &= \mathsf{false}ᵒ \qquad \text{otherwise} 
  \end{align*}
  The $ℰ$ and $𝒱$ functions terminate because the
...
\frame{
  ...
  Proof rules regarding the ``later`` operator ▷ᵒ:
  \begin{gather*}
    \inference{𝒫 ⊢ᵒ ϕ}{𝒫 ⊢ᵒ ▷ᵒ ϕ}
    \quad
    \inference{𝒫 ⊢ᵒ ▷ᵒ (ϕ ⇒ᵒ ψ)}
              {𝒫 ⊢ᵒ ▷ᵒ ϕ ⇒ᵒ ▷ᵒ ψ}
    \quad
    \cdots
    \\[1ex]
    \inference{𝒫 ⊢ᵒ ▷ᵒ ϕ & ϕ,𝒫 ⊢ᵒ ψ}
              {𝒫 ⊢ᵒ ▷ᵒ ψ}
  \end{gather*}
  L\"ob Induction:
  \[
...
%===============================================================================
\frame{
  \frametitle{Recipe: extend $𝒱$ and $ℰ$ to open terms}

    \begin{align*}
      𝒢(A_1,\ldots,A_n)(\sigma) &=
         𝒱(A_1)(\sigma_0), \ldots, 𝒱(A_n)(\sigma_n)   \\
      \Gamma \models^{𝒱} V : A &=
        \forall \sigma,
        𝒢(\Gamma)(\sigma) \mathop{⊢ᵒ} 𝒱(A)(\sigma(V))\\
      \Gamma \models M : A &=
        \forall \sigma,
        𝒢(\Gamma)(\sigma) \mathop{⊢ᵒ} ℰ(A)(\sigma(M))
    \end{align*}
...
\frame{
  ...
  \begin{lemma}[Fundamental] \normalfont
    (1) $\Gamma \vdash^{𝒱} V : A$
        implies $\Gamma \models^{𝒱} V : A$ and\\
    (2) $\Gamma \vdash M : A$ implies $\Gamma \models M : A$.
  \end{lemma}

  Proceed by mutual induction on $\Gamma \vdash^{𝒱} V : A$
  and $\Gamma \vdash M : A$.
...
\frame{
  ...
  \begin{lemma}[Compatibility for $\lam{N}$]
    If $A,Γ \models N : B$,
    then $Γ \models^{𝒱} \lam{N} : A → B$.
  \end{lemma}
  \begin{proof}
    Let $σ$ be a substitution.
    We need to show that
    \[
    𝒢(Γ)(σ) ⊢ᵒ \forallᵒ W, \mathop{▷ᵒ} 𝒱(A)(W)
         \Rightarrowᵒ \mathop{▷ᵒ} ℰ(B)(\mathsf{ext}(σ)(N)[W])
    \]
    Let $W$ be a value and assume $\mathop{▷ᵒ} 𝒱(A)(W)$.\\
    From the premise $A,Γ ⊧ N : B$ we have
    $𝒱(A)(W),𝒢(Γ)(σ) ⊢ᵒ ℰ(B)((W,σ)(N))$.\\
...
\frame{
  ...
  \begin{lemma}[Compatibility for $μ V$]
    If $A→B,Γ \models^{𝒱} V : A → B$,
    then $Γ \models^{𝒱} μ V : A → B$.
  \end{lemma}
  \begin{proof}
    Let $σ$ be a substitution.
    We need to show that \\
    ...
    $\mathop{▷ᵒ} 𝒱(A → B)(μ\,V')$ \qquad\qquad (IH)  \\
    From the premise of this lemma we have \\
    $𝒢(A→B, Γ)(σ') ⊢ᵒ 𝒱(A → B, σ'(V))$ 
    where $σ' = (μ\,V',σ)$. \\
    which is equivalent to \\
    $𝒱(A→B)(μ\,V'),𝒢(Γ)(σ) ⊢ᵒ 𝒱(A → B)(σ'(V))$. \\
    Together with (IH), we have \\
    $𝒢(Γ)(σ) ⊢ᵒ \mathop{▷ᵒ} 𝒱(A → B)(σ'(V))$\\
...
\frame{
  ...
  \begin{proof}
    ...
    $𝒢(Γ)(σ) ⊢ᵒ ℰ(B)(σ(L) \app σ(M))$.\\
    We apply the Bind Lemma to $σ(L)$ and $σ(M)$ to obtain
    $σ(L) \longrightarrow^{*} V$, $σ(M) \longrightarrow^{*} W$,
    $𝒱(A{→}B)(V)$, $𝒱(A)(W)$, and
    it remains to prove $ℰ(B)(V \app W)$,
...
\item Logical Relations Recipe:\\
  Define $𝒱(A)(V)$ and $ℰ(A)(M)$.\\
  Extend $𝒱$ and $ℰ$ to open terms.\\
  Prove the Fundamental Lemma. \\
  Prove that $ℰ$ implies your theorem.
\item A Step-Indexed Logic:\\
  Enables the definition of recursive predicates ($𝒱$ and $ℰ$)
  on full-featured programming languages. \\
  Hides the bookkeeping of the step indexing.\\
  Automates the proofs of monotonicity of $𝒱$ and $ℰ$.
\end{itemize}
}
%===============================================================================
%% \frame{
...
%%     Let $σ$ be a substitution. We need to prove that\\
%%     $𝒢(Γ)(σ) ⊢ᵒ ℰ(B)(σ(L) \app σ(M))$.\\
%%     First we show progress for $σ(L) \app σ(M)$.
%%     From the first premise we have
%%     $𝒢(Γ)(σ) ⊢ᵒ ℰ(A → B)(σ(L))$.
%%     By the progress aspect of $ℰ$, either (1) $σ(L) \longrightarrow L'$ for some $L'$
%%     or (2) $σ(L)$ is a value $V$ with $𝒱(A→B)(V)$.
%%     Suppose (1). Then $σ(L) \app σ(M) \longrightarrow L' \app σ(M)$. \\
%%     Suppose (2). From the second premise we have
%%     $𝒢(Γ)(σ) ⊢ᵒ ℰ(A)(σ(M))$.
%%     By the progress aspect of $ℰ$, either (3) $σ(M) \longrightarrow M'$ for some $M'$
%%     or (4) $σ(M)$ is a value $W$ with $𝒱(A)(W)$. \\
%%     Suppose (3). Then $V \app σ(M) \longrightarrow V \app M'$. \\
%%     Suppose (4). We apply the previous lemma, using $𝒱(A→B)(V)$ and $𝒱(A)(W)$,
%%     to obtain $ℰ(B)(V \app W)$, which implies progress for $V \app W$.

%%   \end{proof}
%% }  
%% %===============================================================================
%% \frame{
%%   \frametitle{Alternative proof, continued}
%%   \small
%%   \begin{lemma}[Compatibility for application]
%%     If $Γ \models L : A → B$ and $Γ ⊧ M : A$,
%%     then $Γ \models L \app M : B$.
...
%%     $𝒢(Γ)(σ) ⊢ᵒ ℰ(B)(σ(L) \app σ(M))$.\\
%%     \vdots 
%%     Next we show preservation for $σ(L) \app σ(M)$.
%%     So we have $σ(L) \app σ(M) \longrightarrow N$
%%     and need to show $▷ᵒ ℰ(B)(N)$. We proceed by cases on the reduction.
