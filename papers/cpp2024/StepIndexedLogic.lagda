...
\begin{code}
  ...
  ×-elim-L : ∀{ϕ}{ψ}{𝒫}{R}
     → ϕ ∷ ψ ∷ 𝒫 ⊢ᵒ R
     → (ϕ ×ᵒ ψ) ∷ 𝒫 ⊢ᵒ R
  ×-elim-L {ϕ}{ψ}{𝒫}{R} ϕψ𝒫⊢R n ((ϕn , ψn) , 𝒫n) = ϕψ𝒫⊢R n (ϕn , (ψn , 𝒫n))
  
  inj₁ᵒ : ∀{𝒫 : List Setᵒ }{ϕ ψ : Setᵒ}
    → 𝒫 ⊢ᵒ ϕ
      ------------
    → 𝒫 ⊢ᵒ ϕ ⊎ᵒ ψ
  ...
  caseᵒ : ∀{𝒫 : List Setᵒ }{ϕ ψ R : Setᵒ}
    → 𝒫 ⊢ᵒ ϕ ⊎ᵒ ψ
    → ϕ ∷ 𝒫 ⊢ᵒ R
    → ψ ∷ 𝒫 ⊢ᵒ R
      ------------
    → 𝒫 ⊢ᵒ R
  caseᵒ 𝒫⊢ϕ⊎ψ ϕ∷𝒫⊢R ψ∷𝒫⊢R n ⊨𝒫n
      with 𝒫⊢ϕ⊎ψ n ⊨𝒫n
  ... | inj₁ ϕn = ϕ∷𝒫⊢R n (ϕn , ⊨𝒫n)
  ... | inj₂ ψn = ψ∷𝒫⊢R n (ψn , ⊨𝒫n)

  case3ᵒ : ∀{𝒫 : List Setᵒ }{ϕ ψ R S : Setᵒ}
    → 𝒫 ⊢ᵒ ϕ ⊎ᵒ (ψ ⊎ᵒ R)
    → ϕ ∷ 𝒫 ⊢ᵒ S
    → ψ ∷ 𝒫 ⊢ᵒ S
    → R ∷ 𝒫 ⊢ᵒ S
      ------------
    → 𝒫 ⊢ᵒ S
  case3ᵒ 𝒫⊢ϕ⊎ψ⊎R ϕ∷𝒫⊢S ψ∷𝒫⊢S R∷𝒫⊢S n ⊨𝒫n
      with 𝒫⊢ϕ⊎ψ⊎R n ⊨𝒫n
  ... | inj₁ ϕn = ϕ∷𝒫⊢S n (ϕn , ⊨𝒫n)
  ... | inj₂ (inj₁ ψn) = ψ∷𝒫⊢S n (ψn , ⊨𝒫n)
  ... | inj₂ (inj₂ Rn) = R∷𝒫⊢S n (Rn , ⊨𝒫n)

  case4ᵒ : ∀{𝒫 : List Setᵒ }{ϕ ψ R S T : Setᵒ}
    → 𝒫 ⊢ᵒ ϕ ⊎ᵒ (ψ ⊎ᵒ (R ⊎ᵒ S))
    → ϕ ∷ 𝒫 ⊢ᵒ T
    → ψ ∷ 𝒫 ⊢ᵒ T
    → R ∷ 𝒫 ⊢ᵒ T
    → S ∷ 𝒫 ⊢ᵒ T
      ------------
    → 𝒫 ⊢ᵒ T
  case4ᵒ 𝒫⊢ϕ⊎ψ⊎R⊎S ϕ∷𝒫⊢T ψ∷𝒫⊢T R∷𝒫⊢T S∷𝒫⊢T n ⊨𝒫n
      with 𝒫⊢ϕ⊎ψ⊎R⊎S n ⊨𝒫n
  ... | inj₁ ϕn = ϕ∷𝒫⊢T n (ϕn , ⊨𝒫n)
  ... | inj₂ (inj₁ ψn) = ψ∷𝒫⊢T n (ψn , ⊨𝒫n)
  ... | inj₂ (inj₂ (inj₁ Rn)) = R∷𝒫⊢T n (Rn , ⊨𝒫n)
  ... | inj₂ (inj₂ (inj₂ Sn)) = S∷𝒫⊢T n (Sn , ⊨𝒫n)

  case-Lᵒ : ∀{𝒫 : List Setᵒ }{ϕ ψ R : Setᵒ}
     → ϕ ∷ 𝒫 ⊢ᵒ R
     → ψ ∷ 𝒫 ⊢ᵒ R
     → ϕ ⊎ᵒ ψ ∷ 𝒫 ⊢ᵒ R
  case-Lᵒ {𝒫} {ϕ} {ψ} {R} ϕ⊢R ψ⊢R n (inj₁ ϕn , 𝒫n) = ϕ⊢R n (ϕn , 𝒫n)
  case-Lᵒ {𝒫} {ϕ} {ψ} {R} ϕ⊢R ψ⊢R n (inj₂ ψn , 𝒫n) = ψ⊢R n (ψn , 𝒫n)

  {- todo: consider making ϕ explicit -Jeremy -}
  →ᵒI : ∀{𝒫 : List Setᵒ }{ϕ ψ : Setᵒ}
    → ϕ ∷ 𝒫 ⊢ᵒ ψ
      ------------
    → 𝒫 ⊢ᵒ ϕ →ᵒ ψ
  →ᵒI {𝒫} ϕ∷𝒫⊢ψ n ⊨𝒫n j j≤n ϕj =
      ϕ∷𝒫⊢ψ j (ϕj , downClosed-Πᵒ 𝒫 n ⊨𝒫n j j≤n)

  appᵒ : ∀{𝒫 : List Setᵒ }{ϕ ψ : Setᵒ}
    → 𝒫 ⊢ᵒ ϕ →ᵒ ψ
    → 𝒫 ⊢ᵒ ϕ
      ------------
    → 𝒫 ⊢ᵒ ψ
  appᵒ {𝒫} 𝒫⊢ϕ→ψ 𝒫⊢ϕ n ⊨𝒫n =
     let ϕn = 𝒫⊢ϕ n ⊨𝒫n in
     let ψn = 𝒫⊢ϕ→ψ n ⊨𝒫n n ≤-refl ϕn in
     ψn
  ...
  ▷→▷ : ∀{𝒫}{ϕ ψ : Setᵒ}
     ...
     → 𝒫 ⊢ᵒ ▷ᵒ ψ
  ▷→▷ {𝒫}{ϕ}{ψ} ▷ϕ ϕ⊢ψ n ⊨𝒫n =
    let ▷ψ = appᵒ{𝒫}{▷ᵒ ϕ}{▷ᵒ ψ}
                (▷→{𝒫}{ϕ}{ψ}
                    (monoᵒ{𝒫}{ϕ →ᵒ ψ} (→ᵒI{𝒫}{ϕ}{ψ} ϕ⊢ψ))) ▷ϕ in
    ▷ψ n ⊨𝒫n

  ⊢ᵒ-∀-intro : ∀{𝒫 : List Setᵒ }{A}{ϕ : A → Setᵒ}
    → (∀ a → 𝒫 ⊢ᵒ ϕ a)
      ----------------------
    → 𝒫 ⊢ᵒ ∀ᵒ ϕ
  ⊢ᵒ-∀-intro ∀ϕa n ⊨𝒫n a = ∀ϕa a n ⊨𝒫n
  ...
  ⊢ᵒ-∀-intro-new : ∀{𝒫 : List Setᵒ }{A}
    ...
    → (∀ a → 𝒫 ⊢ᵒ ϕ a)
      ----------------------
    → 𝒫 ⊢ᵒ ∀ᵒ ϕ
  ⊢ᵒ-∀-intro-new ϕ ∀ϕa n ⊨𝒫n a = ∀ϕa a n ⊨𝒫n

  instᵒ : ∀{𝒫 : List Setᵒ }{A}{ϕ : A → Setᵒ}
    → 𝒫 ⊢ᵒ ∀ᵒ ϕ
    → (a : A)
      ---------
    → 𝒫 ⊢ᵒ ϕ a
  instᵒ ⊢∀ϕ a n ⊨𝒫n = ⊢∀ϕ n ⊨𝒫n a
  ...
  instᵒ-new : ∀{𝒫 : List Setᵒ }{A}
    ...
    → (a : A)
      ---------
    → 𝒫 ⊢ᵒ ϕ a
  instᵒ-new ϕ ⊢∀ϕ a n ⊨𝒫n = ⊢∀ϕ n ⊨𝒫n a

Λᵒ-syntax = ⊢ᵒ-∀-intro
infix 1 Λᵒ-syntax
syntax Λᵒ-syntax (λ a → ⊢ϕa) = Λᵒ[ a ] ⊢ϕa

abstract
  ⊢ᵒ-∃-intro : ∀{𝒫 : List Setᵒ }{A}{ϕ : A → Setᵒ}{{_ : Inhabited A}}
    → (a : A)
    → 𝒫 ⊢ᵒ ϕ a
      ----------
    → 𝒫 ⊢ᵒ ∃ᵒ ϕ
  ⊢ᵒ-∃-intro a ⊢ϕa n ⊨𝒫n = a , (⊢ϕa n ⊨𝒫n)
  ...
  ⊢ᵒ-∃-intro-new : ∀{𝒫 : List Setᵒ }{A}{{_ : Inhabited A}}
    ...
    → 𝒫 ⊢ᵒ ϕ a
      ----------
    → 𝒫 ⊢ᵒ ∃ᵒ ϕ
  ⊢ᵒ-∃-intro-new ϕ a ⊢ϕa n ⊨𝒫n = a , (⊢ϕa n ⊨𝒫n)

  ⊢ᵒ-∃-elim : ∀{𝒫 : List Setᵒ }{A}{ϕ : A → Setᵒ}{R : Setᵒ}{{_ : Inhabited A}}
    → 𝒫 ⊢ᵒ ∃ᵒ ϕ
    → (∀ a → ϕ a ∷ 𝒫 ⊢ᵒ R)
      ---------------------
    → 𝒫 ⊢ᵒ R
  ⊢ᵒ-∃-elim{R = R} ⊢∃ϕ cont zero ⊨𝒫n = tz R
  ⊢ᵒ-∃-elim ⊢∃ϕ cont (suc n) ⊨𝒫n
      with ⊢∃ϕ (suc n) ⊨𝒫n
  ... | (a , ϕasn) = cont a (suc n) (ϕasn , ⊨𝒫n)

  {- making ϕ explicit, inference not working -}
  ⊢ᵒ-∃-elim-new : ∀{𝒫 : List Setᵒ }{A}{R : Setᵒ}{{_ : Inhabited A}}
    ...
    → (∀ a → ϕ a ∷ 𝒫 ⊢ᵒ R)
      ---------------------
    → 𝒫 ⊢ᵒ R
  ⊢ᵒ-∃-elim-new{R = R} ϕ ⊢∃ϕ cont zero ⊨𝒫n = tz R
  ⊢ᵒ-∃-elim-new ϕ ⊢∃ϕ cont (suc n) ⊨𝒫n
      with ⊢∃ϕ (suc n) ⊨𝒫n
  ... | (a , ϕasn) = cont a (suc n) (ϕasn , ⊨𝒫n)
  ...
  ⊢ᵒ-∃-elim-L : ∀{𝒫 : List Setᵒ }{A}{R : Setᵒ}{{_ : Inhabited A}}
    ...
    → (∀ a → ϕ a ∷ 𝒫 ⊢ᵒ R)
      ---------------------
    → (∃ᵒ ϕ) ∷ 𝒫 ⊢ᵒ R
  ⊢ᵒ-∃-elim-L {R = R} ϕ ϕa⊢R n ((a , ϕan) , ⊨𝒫n) = ϕa⊢R a n (ϕan , ⊨𝒫n)
...
abstract
  ...
  {- same as Sᵒ 
  ⊢ᵒ-weaken : ∀{𝒫 : List Setᵒ}{ϕ R : Setᵒ}
    → 𝒫 ⊢ᵒ R
    → ϕ ∷ 𝒫 ⊢ᵒ R
  ⊢ᵒ-weaken ⊢R n (ϕn , 𝒫n) = ⊢R n 𝒫n
  -}
  ...
  →ᵒI′ : ∀{𝒫 : List Setᵒ }{ϕ ψ : Setᵒ}
    → (ϕ ∷ 𝒫 ⊢ᵒ ϕ → ϕ ∷ 𝒫 ⊢ᵒ ψ)
      -----------------------
    → 𝒫 ⊢ᵒ ϕ →ᵒ ψ
  →ᵒI′ {𝒫}{ϕ}{ψ} ϕ→ψ = →ᵒI{𝒫}{ϕ}{ψ} (ϕ→ψ (Zᵒ{𝒫}{ϕ}))

λᵒ-syntax = →ᵒI′
infix 1 λᵒ-syntax
syntax λᵒ-syntax (λ ⊢ϕ → ⊢ψ) = λᵒ[ ⊢ϕ ] ⊢ψ
...
abstract
  ...
  caseᵒ-L : ∀{𝒫 : List Setᵒ }{ϕ ψ R : Setᵒ}
    → ϕ ∷ 𝒫 ⊢ᵒ R
    → ψ ∷ 𝒫 ⊢ᵒ R
      ------------------
    → (ϕ ⊎ᵒ ψ) ∷ 𝒫 ⊢ᵒ R
  caseᵒ-L{𝒫}{ϕ}{ψ}{R} ϕ∷𝒫⊢R ψ∷𝒫⊢R =
      let 𝒫′ = ϕ ∷ ψ ∷ (ϕ ⊎ᵒ ψ) ∷ 𝒫 in
      let ⊢ϕ⊎ψ : (ϕ ⊎ᵒ ψ) ∷ 𝒫 ⊢ᵒ ϕ ⊎ᵒ ψ
          ⊢ϕ⊎ψ = Zᵒ{𝒫}{ϕ ⊎ᵒ ψ} in
      let ϕ⊢R = ⊢ᵒ-swap{𝒫}{R}{ϕ ⊎ᵒ ψ}{ϕ}
            (Sᵒ{ϕ ∷ 𝒫}{R}{ϕ ⊎ᵒ ψ} ϕ∷𝒫⊢R) in
      let ψ⊢R = ⊢ᵒ-swap{𝒫}{R}{ϕ ⊎ᵒ ψ}{ψ}
            (Sᵒ{ψ ∷ 𝒫}{R}{ϕ ⊎ᵒ ψ} ψ∷𝒫⊢R) in
      caseᵒ{(ϕ ⊎ᵒ ψ) ∷ 𝒫}{ϕ}{ψ}{R} ⊢ϕ⊎ψ ϕ⊢R ψ⊢R
