...
\input{../../src/latex/RawSetO.tex}



Unfortunately, the \textsf{fixpoint}ᵒ theorem that we introduced in
\S~\ref{sec:formula-equivalence} cannot be proved for arbitrary
functions from ℕ to \textsf{Prop}. As usual, one needs to place some
restrictions on the function space. We place three restrictions on
these functions: (1) they must be downward closed




The primary challenge in constructing the Step-Indexed Logic is in
proving 
...
\input{../../src/latex/EquivalenceRelation.tex}
